...
def get_distribution_stats(solver_name, alpha_value, kills, hypoxic_threshold_list, plot=0, read=0):    
    ...
    # Set the file path
    folder_path = main_folder_path + solver_name + 'Haematocrit/Lambda4/Alpha' + alpha_value + '/Kills' + kills
#    folder_path = '/scratch/narain/Stochastic Pruning in Forking Network with 100 Trials adjusted/TestDichotomousNetwork/' + solver_name + 'Haematocrit/Lambda4/Alpha' + alpha_value + '/Selection' + layout_selection + '/Beta' + beta
    ...
    if read==0:    
        ...
        n_vessels, n_perfused_vessels, n_unperfused_vessels, mean_diameter, mean_geometric_resistance, diameter_adjacency_matrix, length_adjacency_matrix = get_forking_predictors(network_path, reference_rank_lengths, reference_node_coordinates, pq_threshold)
        '''
...
main_folder_path = '/home/narain/Desktop/Results/Forking/Nonrandom/Individual Pruning in Nonrandom Forking Network/TestDichotomousNetwork/'
...
for i in range(len(alpha_list)):
    axs[i].plot(alpha_array[:,1], n_vessels_composite[i], label='total')
    axs[i].plot(alpha_array[:,1], n_perfused_vessels_composite[i], ls='dashed', label='perfused')
    axs[i].plot(alpha_array[:,1], n_unperfused_vessels_composite[i], ls='dotted', label='unperfused')
    axs[i].set_xlim(0)
...
# Save image
file_path = Path('~/Desktop/Final Figures/' + solver_name + '_nonrandom_forking_individual_pruning_geometric_metrics.svg').expanduser()
fig.savefig(file_path, dpi=500, bbox_inches = 'tight')
file_path = Path('~/Desktop/Final Figures/' + solver_name + '_nonrandom_forking_individual_pruning_geometric_metrics.png').expanduser()
fig.savefig(file_path, dpi=500, bbox_inches = 'tight')
