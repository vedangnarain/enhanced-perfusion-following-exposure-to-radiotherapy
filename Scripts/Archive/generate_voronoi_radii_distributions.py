...
# -*- coding: utf-8 -*-
"""
Created on Mon Aug  8 11:46:49 2022

@author: Vedang Narain (vedang.narain@msdtc.ox.ac.uk)

Generate truncated normal and log normal distributions for Voronoi vessel radii.
...
n_vessels = 393-(n_inlets+n_outlets)
'''
...
#plt.suptitle('Voronoi Network Vessel Radius Distribution (μ = ' + str(mean) + ' μm, n_vessels = ' + str(n_vessels) + ', min = ' + str(lower_bound) + ' μm, max = ' + str(upper_bound) + ' μm)')
...
#    axs[i].legend()
#    axs[i].grid()
    axs[i].title.set_text('${µ}$ = ' + str(mean_list[i]) + ' μm')
    axs[i].tick_params(labelbottom=True)
    axs[i].axvline(mean_list[i], c='black', ls='--', label='mean')
    axs[i].set_ylim(0, 75)
    if i==0:
        axs[i].legend()
plt.show()
...
for mu_number in mean_list:
    for list_number in range(1,101):
        id_name = '/home/narain/Chaste/projects/MicrovesselChaste/test/simulation/flow/100SeedPoints/square_voronoi_radius_log_normal_distribution/mu_' + str(mu_number) + '/id_list_' + str(list_number) + '.txt'
        radii_name = '/home/narain/Chaste/projects/MicrovesselChaste/test/simulation/flow/100SeedPoints/square_voronoi_radius_log_normal_distribution/mu_' + str(mu_number) + '/radii_list_' + str(list_number) + '.txt'
        composite_list = generate_log_normal_distribution(mu_number, add_ids=True)  
        sorted_composite_list = composite_list
...
# Save image
file_path = Path('/home/narain/Chaste/projects/MicrovesselChaste/test/simulation/flow/100SeedPoints/square_voronoi_radius_log_normal_distribution/voronoi_radius_log_normal_distribution.svg').expanduser()
fig.savefig(file_path, dpi=500)
file_path = Path('/home/narain/Chaste/projects/MicrovesselChaste/test/simulation/flow/100SeedPoints/square_voronoi_radius_log_normal_distribution/voronoi_radius_log_normal_distribution.png').expanduser()
fig.savefig(file_path, dpi=500)
#'''
