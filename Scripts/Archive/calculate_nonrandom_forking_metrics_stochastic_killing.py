...
#        print(layout_selection)
        hypoxic_fraction_list, mean_value, min_value, half_value, max_value, std_value, n_vessels_value, mean_diameter_value, mean_geometric_resistance_value, diameter_adjacency_matrix_value, length_adjacency_matrix_value = get_distribution_stats(solver_name, str(trial_selection), alpha_value, str(beta), hypoxic_threshold_list, plot, read)
        table_entry = np.hstack([hypoxic_fraction_list, mean_value, min_value, half_value, max_value, std_value, n_vessels_value, mean_diameter_value, mean_geometric_resistance_value])
...
'''
solver_stats = np.delete(solver_stats, 3, 1)

FIND A WAY TO COMBINE TWO HF'S AS ONE COLUMN SO IT FITS. MAKE SURE HF PLOTS ARE ALSO WORKING THOUGH.

ALSO STORE ADJACENCY MATRICES SOMEHOW OR EXPORT IN SOME OTHER WAY.
...
# Save image
file_path = Path('~/Desktop/Final Figures/' + solver_name + '_nonrandom_forking_stochastic_pruning_geometric_metrics.svg').expanduser()
fig.savefig(file_path, dpi=500, bbox_inches = 'tight')
file_path = Path('~/Desktop/Final Figures/' + solver_name + '_nonrandom_forking_stochastic_pruning_geometric_metrics.png').expanduser()
fig.savefig(file_path, dpi=500, bbox_inches = 'tight')
