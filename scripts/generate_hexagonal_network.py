...
# Set the file name
file_name = '/home/narain/Chaste/projects/MicrovesselChaste/test/simulation/flow/100VesselLength/LongEdgesMatrix_Offset.txt'

# Enter the network specifications
side_length = 100
x_start = 0
x_end = 4200
y_start = 0
...
x_upper = 3850
y_upper = 1000
edge_matrix = edge_matrix[np.logical_and(edge_matrix[:,0] >= x_start, edge_matrix[:,2] >= x_start)]
...
edge_matrix = edge_matrix[edge_matrix[:, 2].argsort(kind='mergesort')]  # sort by month
edge_matrix = edge_matrix[edge_matrix[:, 1].argsort(kind='mergesort')]  # sort by month
edge_matrix = edge_matrix[edge_matrix[:, 0].argsort(kind='mergesort')]  # sort by month
