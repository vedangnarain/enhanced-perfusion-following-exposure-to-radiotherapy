...
# Set folder path
main_folder_path = '/home/narain/Desktop/Results/Forking/Nonrandom/Individual Pruning in Nonrandom Forking Network with Varying Means (' + str(n_gens) + ' Gens ' + str(d_inlet) + ' Inlet)/TestDichotomousNetwork/'
...
#    axs[row_index].set_ylim([0,1.1])  # set PQ limits
    axs[row_index].plot(mean_array[:,1], mean_geometric_resistance_composite[i-offset*row_index]*10000, ls = linestyles[i-offset*row_index], c=linecolours[i-offset*row_index], label = graph_mean_list[i-offset*row_index], lw=3)
    axs[row_index].set_xlim(0,250)
...
axs.plot(mean_array[:,1], loops_per_vessel[which_mean]*100, label = r'$\overline{\beta_1}$', c='C3', lw=3)
axs.set_xlim(0,250)
axs.set_ylim([0,30])
...
ax2.plot(mean_array[:,1], (pq_composite[which_mean,:]-pq_composite[which_mean,1])*100/pq_composite[which_mean,1], label='$\Delta_{\%} \mathcal{P}$', c='C0', lw=3)
...
#    pq_composite[:,0]=pq_composite[:,1]  # fix flow rate error
    pq_composite[i-offset*row_index,0]=pq_composite[i-offset*row_index,1]  # fix flow rate error
    axs[row_index].plot(mean_array[:,1], pq_composite[i-offset*row_index,:], ls = linestyles[i-offset*row_index], c=linecolours[i-offset*row_index], label = graph_mean_list[i-offset*row_index], lw=3)
    axs[row_index].set_xlim(0,250)
    axs[row_index].set_ylim(0,1)    
    axs[row_index].set_xlabel('vessels pruned')    
    axs[row_index].set_ylabel(r'$\mathcal{P}$') 
#    axs[row_index].legend(title='mean diameter', title_fontsize='14', fontsize='14')
    # axs[row_index].legend()
    axs[row_index].grid()
    axs[row_index].title.set_text('${α}$ = ' + graph_alpha_list[which_alpha]) 
...
axs.plot(mean_array[:,1], n_perfused_vessels_composite[x], ls='dashed', label='perfused', c='g', lw=3)
axs.plot(mean_array[:,1], n_unperfused_vessels_composite[x], ls='dotted', label='hypoperfused', c='r', lw=3)
# axs.legend()
axs.set_xlim(0,250)
axs.set_ylim(0,250)
# axs.set_xlabel(r'vessels pruned $({N}_P)$')    
axs.set_xlabel('vessels pruned')    
...
ax2.plot(vessels_axis, relative_pf, label='$\Delta_{\%} \mathcal{P}$', c='C0', lw=3)
...
for i in range(len(mean_list)*row_index,len(mean_list)*(row_index+1)):
    ...
    axs[row_index].plot(mean_array[:,1], resistance_per_loop*100, ls = linestyles[i-offset*row_index], c=linecolours[i-offset*row_index], label = graph_mean_list[i-offset*row_index], lw=3)
    axs[row_index].set_xlim(0,250)
    axs[row_index].set_ylim(pow(10,-2),pow(10,1))
    axs[row_index].set_xlabel('vessels pruned')    
    if which_alpha==1:
...
#    axs[row_index].set_ylim([0,100])  # set PQ limits
    axs[row_index].plot(mean_array[:,1], (pq_composite[int(mean_list[0]),:]-pq_composite[int(mean_list[0]),1])*100/pq_composite[int(mean_list[0]),1], label = r'$\overline{d}$ = ' + graph_mean_list[int(mean_list[0])], c=colors[0])
...
#    axs[row_index].grid()
    axs[row_index].spines['right'].set_visible(False)
    axs[row_index].spines['top'].set_visible(False)
#    axs[row_index].title.set_text(r'$\overline{d}$ = ' + graph_mean_list[row_index])
row_index+=1
