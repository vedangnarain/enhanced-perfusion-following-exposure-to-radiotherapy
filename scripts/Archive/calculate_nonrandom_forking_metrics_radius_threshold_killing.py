...
for i in range(len(alpha_list)):
    axs[i].plot(alpha_array[:,1]*2, n_vessels_composite[i], ls='dashed')
    axs[i].set_xlim(0)
...
for i in range(len(alpha_list),len(alpha_list)*2):
    axs[i].set_ylim([0,1.1])  # set PQ limits
    axs[i].plot(alpha_array[:,1]*2, pq_composite[i-offset,:])
    axs[i].set_xlim(0)
#    axs[i].set_xlabel('radius threshold (μm)')    
    if i==len(alpha_list):
        axs[i].set_ylabel('PQ') 
...
# Save image
file_path = Path('~/Desktop/Final Figures/' + solver_name + '_nonrandom_forking_radius_threshold_pruning_geometric_metrics.svg').expanduser()
fig.savefig(file_path, dpi=500, bbox_inches = 'tight')
file_path = Path('~/Desktop/Final Figures/' + solver_name + '_nonrandom_forking_radius_threshold_pruning_geometric_metrics.png').expanduser()
fig.savefig(file_path, dpi=500, bbox_inches = 'tight')
